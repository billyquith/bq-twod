...
namespace cp {
    ...
    public:
        ...
        std::shared_ptr<Shape> segmentQueryFirst(Vect a, Vect b, Layers, Group,
                                                 SegmentQueryInfo* = nullptr) const;
    ...
    private:
        ...
        struct CallbackData
        {
            ...
            CallbackData(std::function<int(Arbiter, Space&)> begin,
                         std::function<int(Arbiter, Space&)> preSolve,
                         std::function<void(Arbiter, Space&)> postSolve,
                         std::function<void(Arbiter, Space&)> separate,
                         Space& self)
        ...
        std::map<std::pair<CollisionType, CollisionType>,
                 std::unique_ptr<CallbackData> > callbackDatas;
